 ...
 class RunningOrdersArrayAdapter extends ArrayAdapter<BitOrder> {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
       ...
       // BitOrder ord = getItem(position);
        ...
        try {
            precio.setText(String.format(Locale.getDefault(),"%.8f",getItem(position).getOrderMxnPrice()));
        } catch (JSONException e) {
            e.printStackTrace();
        }
        ...
        try {
            ...
            volumen.setText(String.format(Locale.getDefault(),"%.8f",getItem(position).getOrderAmount()));
        } catch (JSONException e) {
            e.printStackTrace();
        }
